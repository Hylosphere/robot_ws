...
# NOUVEAU : Variable pour désactiver Git en local
SKIP_GIT="${SKIP_GIT:-0}"
...
# === MODIFICATION ICI ===
if [ "${SKIP_GIT}" = "1" ]; then
    log "MODE DEV DÉTECTÉ : On saute l'étape Git (Dossier monté depuis l'hôte)."
else
    # --- DEBUT BLOC GIT ORIGINAL ---
